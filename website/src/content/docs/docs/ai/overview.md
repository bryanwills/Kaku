...
| `model` | 错误修复和自然语言转命令使用的模型 |
| `chat_model` | `Cmd + L` AI Chat 使用的模型 |
| `chat_model_choices` | AI Chat 可切换模型列表 |
| `base_url` | 兼容 OpenAI 协议的 API 根地址 |
| `custom_headers` | 用于企业代理的额外 HTTP 头，例如 `["X-Customer-ID: your-id"]` |
| `web_search_provider` | 可选，AI Chat 网页搜索 Provider |
| `web_search_api_key` | 可选，网页搜索 Provider 的 API Key |
